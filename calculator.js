angular
  ...
  .controller("CalculatorController", function ($scope, $timeout) {
    ...
    vm.updateFilteredSelections = function () {
      const selectedHouse = vm.selections.house
        ? vm.selections.house.name
        : null;
      const selectedExtensions = vm.selections.extensions.map(
        (ext) => ext.name
      );
      ...
      vm.filteredSelections = vm.finalSelections.filter((item) => {
        const houseMatches =
          !selectedHouse || item.houseType.includes(selectedHouse);
        const extensionsMatch =
          selectedExtensions.length === 0 ||
          item.extensionType.some((ext) => selectedExtensions.includes(ext));
        const roofsMatch =
          selectedRoofs.length === 0 ||
          item.roofType.some((roof) => selectedRoofs.includes(roof));
    ...
    function getPossibleRoofs(houseType, extensionNames) {
      ...
      for (let item of vm.finalSelections) {
        if (
          item.houseType.includes(houseType) &&
          extensionNames.some((extName) => item.extensionType.includes(extName))
        ) {
    ...
    vm.selectHouseType = function (house) {
      ...
      if (vm.selections.house) {
        ...
        vm.extensionTypes = vm.originalExtensionTypes.filter((ext) =>
          possibleExtensions.includes(ext.name)
        );
      }
    ...
    vm.selectExtensionType = function (extension) {
      ...
      if (vm.selections.house && vm.selections.extensions.length) {
        ...
        vm.roofTypes = vm.originalRoofTypes.filter((roof) =>
          possibleRoofs.includes(roof.name)
        );
      }
      vm.updateFilteredSelections();
